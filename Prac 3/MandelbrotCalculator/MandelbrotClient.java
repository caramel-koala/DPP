...
public class MandelbrotClient extends Applet
  ...
  { private int xsize;      // dimensions of window
      ...
      { xsize = getSize().width;
        ...
        try
          { Registry registry = LocateRegistry.getRegistry();
      	    calc = (MandelbrotCalculator)registry.lookup("MandelbrotService");
      	  }
        catch (Exception e)
    ...
    private void generateImage()
      { byte[][] image = null;
        ...
        try
          { image = calc.calculateMandelbrot(xsize, ysize, x1, x2, y1, y2);
          }
    ...
    private void display (byte[][] points)
      { int j = 0;
        for(int l=0;j < ysize; j++, l++)
